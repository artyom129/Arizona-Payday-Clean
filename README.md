...
- снижает фоновую нагрузку на процессор в версии 2.0.21.
...
## 🚑 Что нового в версии 2.0.21

- исправлен реальный Payday со строками `$ 44.466.721 (+$ 904.545)` и `$ 295.391.104 (+$ 689.244)`;
- парсер больше не зависит от точного положения `$`, пробелов и служебных символов;
- учитывается временная метка `[HH:MM:SS]`, если клиент добавил её в текст события;
- исправлено распознавание строки `Общая заработная плата`;
- усилено распознавание талона с игровым значком перед словом `Талон`;
- быстрый частичный отчёт не отправляется, если распознан только один показатель;
- неизвестные банк и депозит больше не подменяются старыми значениями в Telegram.

---

## ⚡ Оптимизация версии 2.0.20

Версия **2.0.21** посвящена снижению нагрузки на компьютер при работе GTA в фоне.
...
В версии 2.0.21 используется адаптивная задержка, отключение рендера мини-окна в фоне и более длинный Telegram long polling.
...
```text
Тег: v2.0.21
Название: Arizona Payday Clean v2.0.21
...
Готовое описание релиза находится в `releases/v2.0.21.md`.
