...
local function parseTicketAmounts(text, nums)
    ...
    -- Количество берём только после слова «Талон» и символа + перед AZ Coin(s).
    -- Произвольное первое число строки больше не используется.
    local plusRaw = source:match(ticketPattern .. '.-%+%s*([%d%s%.,]+)%s*' .. coinPattern)
        or source:match(ticketPattern .. '%s*:%s*[%+xX]%s*([%d%s%.,]+)')
    local balanceRaw = source:match('%(%s*([%d%s%.,]+)%s*[Шш][Тт]%.?%s*%)')
...
local function isTicketNotification(text)
    ...
    local hasTicketWord = source:match('[Тт][Аа][Лл][Оо][Нн]') ~= nil
    local hasAzWord = asciiLower:match('az%s*coins?') ~= nil
    local hasAward = source:match('[Тт][Аа][Лл][Оо][Нн].-%+%s*%d') ~= nil
        or source:match('[Тт][Аа][Лл][Оо][Нн].-[Вв][Сс][Ее][Гг][Оо]') ~= nil
        or source:match('[Тт][Аа][Лл][Оо][Нн].-[Бб][Аа][Лл][Аа][Нн][Сс]') ~= nil
...
-- Точные разборщики Payday работают только с частью строки после двоеточия.
-- Это исключает ID, номер ранга и другие числа из названия сообщения.
