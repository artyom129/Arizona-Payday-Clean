...
local function telegramMessage()
    ...
    local lines = {
        'PayDay получен',
        '',
        'Зарплата: ' .. money(salary),
        'Бонус: ' .. multiplierText(st.multiplier),
        'Доход с депозита: ' .. money(depositPlus),
        'Всего за PayDay: ' .. money(totalPayday),
        '',
        'Банк: ' .. money(ini.stats.bank),
        'Депозит: ' .. money(ini.stats.deposit),
        'AZ Coins: ' .. tostring(ini.stats.az_balance or 0),
        '',
        'Окупаемость ранга',
        'Прогресс: ' .. string.format('%.1f%%', st.progress * 100),
        'Осталось: ' .. money(st.remaining),
        'Осталось ПД x1: ' .. tostring(st.remainingX1),
        'Осталось реальных ПД: ' .. tostring(st.remainingReal)
    }

    if asBool(ini.rank.completed) then
        table.insert(lines, '')
        table.insert(lines, 'Ранг полностью окуплен')
    elseif not asBool(ini.rank.tracking) then
        table.insert(lines, '')
        table.insert(lines, 'Отсчет окупаемости сейчас выключен')
    end
...
local function urlEncodeUtf8(value)
    local utf8Text = u8(tostring(value or ''))
...
local function telegramTestCommand()
    sendTelegramMessage(
        'Arizona PayDay Helper подключен\n\nБот готов присылать информацию после каждого PayDay.',
        true
    )
end
...
function sampev.onServerMessage(color, text)
    ...
    if hasText(text, 'Баланс на донат-счете') or hasText(text, 'AZ') then
        if hasText(text, 'донат-счете') then
...
local miniFrame = imgui.OnFrame(function()
    local st = rankStats()
    return miniEnabled[0] and st.cost > 0 and st.baseSalary > 0
end, function()
